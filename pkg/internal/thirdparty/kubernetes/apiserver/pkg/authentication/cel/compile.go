...
const (
	...
	// MODIFICATION: adding a `response` variable for external claim source response handling
...
func mustBuildEnvs(baseEnv *environment.EnvSet) map[string]*environment.EnvSet {
	...
	// MODIFICATION: Add `response` variable type to the env set.
	responseType := apiservercel.NewMapType(apiservercel.StringType, apiservercel.AnyType, -1)
